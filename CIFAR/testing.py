...
def set_args(args):
    ...
    if args.test == "pgd":
        args.epsilon = args.epsilon/config.std
...
def main():
    ...
    normalised = False if args.test=="autoattack" or args.test=="clever" else True
    config.test_dataloader = models.get_cifar10_test_dataloader(args,normalised)

    model = models.get_cifar10_architecture(args)
    model.load_state_dict(t.load(args.dir + args.name))

    if args.test=="autoattack" or args.test=="clever":
    ...
    if args.test == "autoattack":
        ...
        m = PyTorchClassifier(model, F.cross_entropy, (32, 32, 3), 10)
